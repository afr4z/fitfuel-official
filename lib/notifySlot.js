...
const TEMPLATE_NAMES = {
  breakfast: process.env.WHATSAPP_BREAKFAST_TEMPLATE || "breakfast_notification",
  lunch: process.env.WHATSAPP_LUNCH_TEMPLATE || "lunch_notification",
  dinner: process.env.WHATSAPP_DINNER_TEMPLATE || "dinner_notification",
  rescheduled: process.env.WHATSAPP_RESCHEDULED_TEMPLATE || "rescheduled_meal",
};
...
function getTemplateParams(slot, deliveryDate, itemName, timeStr, deadlineMsg, expiryNotice) {
  return [
    { type: "text", text: deliveryDate },                    // {{1}} = delivery_date
    { type: "text", text: itemName },                        // {{2}} = item_name
    { type: "text", text: timeStr },                         // {{3}} = time_str
    { type: "text", text: deadlineMsg },                     // {{4}} = deadline_msg
    { type: "text", text: expiryNotice },                    // {{5}} = expiry_notice
  ];
}

function getRescheduledParams(slot, slotLabel, slotEmoji, itemName, deliveryDate) {
  return [
    { type: "text", text: slotLabel },      // {{1}} = slot_label
    { type: "text", text: slotEmoji },      // {{2}} = slot_emoji
    { type: "text", text: itemName || "Your meal" }, // {{3}} = item_name
    { type: "text", text: deliveryDate },   // {{4}} = delivery_date
  ];
}
  ...
  const results = await Promise.allSettled(
    activeSubs.map(async (sub) => {
      ...
      const fallbackButtons = [
        ...
        { id: `SKIP_${order.id}`, title: "Skip" },
      ];
      const templateParams = getTemplateParams(slot, deliveryDate, order.item_name, timeStr, deadlineMsg, expiryNotice);
  ...
  for (const ord of dayOrders || []) {
    ...
    try {
      ...
      const fallbackButtons = [
        ...
        { id: `SKIP_${ord.id}`, title: "Skip" },
      ];
      const templateParams = getRescheduledParams(slot, slotLabel, slotEmoji, itemName, deliveryDate);
