...
export async function getMenuItems({ limit = 10, mealPlanId, slot } = {}) {
  let query = supabase
    .from("dishes")
  ...
  console.log(`[PETPOOJA] getMenuItems: fetched ${data?.length ?? 0} dishes (limit=${limit}, mealPlanId=${mealPlanId})`);
  return data.map((d) => ({
    itemid: d.id,
    itemname: d.name,
